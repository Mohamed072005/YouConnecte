                ...
                <div class="col-4 d-flex justify-content-center p-2">
                    <div class="bg-secondary w-75 rounded-pill text-center text-light">
                        <form action="{{ route('like.action', $posts->id) }}" method="post">
                            @csrf
                            <button type="submit" class="text-light" style="background-color: transparent; border: none;">like: {{ $postLikes }}</button>
                        </form>
                    </div>
                </div>
                <div class="col-4 d-flex justify-content-center p-2">
                    <div class="bg-secondary w-75 rounded-pill text-center"><a href="{{ route('get.comments', $posts->id) }}" class="navbar-brand text-light">commente</a></div>
                </div>
                <div class="col-4 d-flex justify-content-center p-2">
                ...
                <div class="row d-flex justify-content-center mt-3 w-100">
                    <div class="col-lg-9">
                        <div class="mb-4 posts-container rounded">
                            ...
                            <div class="mb-2">
                                <div>
                                    <h4 class="text-light">{{ $commentsInfo->user_name }}</h4>
                                </div>
                            ...
                            <form action="{{ route('store.comment') }}" method="post" class="row">
                                @csrf
                                <input type="hidden" name="post_id" value="{{ $posts->user_id }}">
